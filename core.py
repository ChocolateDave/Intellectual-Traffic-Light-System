...
import env as Env
import argparse
from brain import Naive_DQN, Dueling_DQN, OWM_DQN
...
class BaseAgent(object):
    ...
    def __init__(self, config, env):
        ...
        self.model_dir = "%s-%s-%s-%s-%s/" % (os.environ['USERNAME'], config.env_name,\
            config.backend, config.model, str(int(time.time())))
...
if __name__ == "__main__":
    ...
    """
    # 初始化：设置调整方法论的句柄 Init: Set argparse
    parser = argparse.ArgumentParser()
    parser.add_argument('--appr', default='dqn', type=str, dest='appr', help='Custom training approach')
    ...
    env = Env.TrafficLight_v1(SumoConfigs)
    if args.appr == 'dqn':
        agent = DQNAgent(DQNConfigs, env)
    elif args.appr == 'dueling':
        agent = DuelingAgent(DQNConfigs, env)
    elif args.appr == 'owm':
        pass

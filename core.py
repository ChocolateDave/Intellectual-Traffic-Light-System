...
class DeepNetwork:
    ...
    def _build_net(self,inputs,regularizer):
        ...
        with tf.variable_scope('eval_net'):
            ...
            with tf.variable_scope('l1-conv1'):
                ...
                conv1_weights = tf.get_variable('weight', [5, 5, 1, 6],
                                                initializer=tf.truncated_normal_initializer(stddev=0.1), collections=c_names)
                conv1_biases = tf.get_variable('bias', [6], initializer=tf.constant_initializer(0.0), collections=c_names)
                conv1 = tf.nn.conv2d(inputs, conv1_weights, strides=[1, 2, 2, 1], padding='VALID')
            ...
            # eval_net 的第二层. collections 是在更新 target_net 参数时会用到.第二层为池化层，步长为2，不够用0补全。这里的input是116x104
            ...
            with tf.variable_scope('l3-conv2'):
                conv2_weights = tf.get_variable('weight', [5, 5, 6, 16],
                                                initializer=tf.truncated_normal_initializer(stddev=0.1), collections=c_names)
                conv2_biases = tf.get_variable('bias', [16], initializer=tf.constant_initializer(0.0), collections=c_names)
                conv2 = tf.nn.conv2d(pool1, conv2_weights, strides=[1, 2, 2, 1], padding='VALID')
            ...
            with tf.variable_scope('l5-fc1'):
                ...
                fc1_biases = tf.get_variable('bias', [512], initializer=tf.constant_initializer(0.1), collections=c_names)
        ...
        with tf.variable_scope('target_net'):
            ...
            # target_net 的第一层. collections 是在更新 target_net 参数时会用到,n_l1=256是我决定的
            with tf.variable_scope('l1-conv1'):
                # 第一层为卷积层，过滤器大小为5x5，深度为6，不够用0补全。步长为1。
            ...
            # target_net 的第二层. .第二层为池化层，步长为2，不够用0补全。
            with tf.variable_scope('l2-pool1'):
                pool1 = tf.nn.max_pool(relu1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
                # conv1 = tf.layers.con2d(l1,filters=16,kernel_size=5,strides=1,padding='same',activation=tf.nn.relu)
                # w2 = tf.get_variable('w2', [n_l1, self.n_actions], initializer=w_initializer, collections=c_names)
            ...
            with tf.variable_scope('l6-fc2'):
                ...
                # if regularizer != None:
                #     tf.add_to_collection('losses', regularizer(fc2_weights))
                fc2_biases = tf.get_variable('bias', [256], initializer=tf.truncated_normal_initializer(stddev=0.1), collections=c_names)
                fc2 = tf.nn.relu(tf.matmul(fc1, fc2_weights) + fc2_biases)
            ...
            with tf.variable_scope('l7-fc3'):
                ...
                # if regularizer != None:
                #     tf.add_to_collection('losses', regularizer(fc3_weights))
                fc3_biases = tf.get_variable('bias', [8], initializer=tf.truncated_normal_initializer(stddev=0.1), collections=c_names)
                self.q_next = tf.nn.relu(fc2, fc3_weights) + fc3_biases
            return self.q_next
